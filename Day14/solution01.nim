# Sam Shepard - 2022
# First time Nim!
## nim c -r solution01.nim input.txt 
...
proc print_test_grid(grid: seq[seq[char]]) =
    for line in grid[0..9]:
        echo cast[string](line[494..503])

let data = readFile(filename).split("\n").filterIt(it.len != 0).map(line =>
        line.split(" -> ").map(pair => pair.split(",").map(str_num => parseInt(str_num))))
        
let max_c = data.map(coords => coords.map(pair => pair[0]).max).max + 1
let max_r = data.map(coords => coords.map(pair => pair[1]).max).max + 1
