...
// Held-Karp solution to Traveling Salesman Problem
...
for (int subset_size = 2; subset_size < n; subset_size++)
{
    foreach (int subset in GetBitPermutation(subset_size, n))
    {
        ...
        foreach (int p in positions)
        {
            ...
            //var e = positions.Where(m => m != 0 && m != p);
            //var l = e.Select(m => AccumulateCost(partials[previous, m], distances[m, p])).ToList();
            //Console.WriteLine("M found: \n" + String.Join("\n", l));
            //Console.WriteLine($"SET: partials[{Convert.ToString(subset, 2).PadLeft(8, '0')}, {p}]");
        }
    }
}
